...
if __name__ == "__main__":
    ...
    log_file = os.path.join(
        cwd, 'logs', f"{start_datetime.strftime('%Y-%m-%dT%H:%M:%S')}.log")
    ...
    to_remove = list(
        filter(lambda f: file_today(f, start_datetime), log_files))
    for f in to_remove:
    ...
    for key, database_id in conf['DATABASES'].items():
        ...
        db = Database(database_id, notion_token, apple_calendar,
                      folder=os.path.join(cwd, 'databases'))
        db.run()
