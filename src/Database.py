...
class Database():
    ...
    def __init__(self, id: str, notion_token: str, apple_calendar: str, folder=os.getcwd()) -> None:
        ...
        if os.path.exists(self._path):
            self.df = pd.read_csv(self._path, index_col='id', parse_dates=[
                                  'last_edit', 'start_date', 'end_date'])
        else:
            self.df = self._empty()
            self.save()
    ...
    def get_outdated(self) -> List:
        ...
        """
        outdated = list(
            self.df[self.df.end_date.dt.date < datetime.now().date()].index)
    ...
    def get_modified(self, current=None) -> List:
        ...
        _filter = _filter & ((_joined.title != _joined.title_stored) | (
            _joined.start_date != _joined.start_date_stored) | (_joined.end_date != _joined.end_date_stored))
        _filter = _filter & (~ pd.isnull(_joined.last_edit)) & (
            ~ pd.isnull(_joined.last_edit_stored))
    ...
    def modify_events(self, list_id: List, current=None) -> None:
        ...
        for id in list_id:
            ...
            new_event_id = self.calendar_client.add_event(
                *self._get_card_for_calendar(modified_event))
    ...
    def add_events(self, list_id: List, current=None) -> None:
        ...
        for id, s in to_add.iterrows():
            try:
                event_id = self.calendar_client.add_event(
                    *self._get_card_for_calendar(s))
                s['event_id'] = event_id
